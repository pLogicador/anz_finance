...
- **`POST /workspace/upload` validado de ponta a ponta com sucesso real** — bridge (token do agendador → JWT próprio do ANZ Finance) seguido de upload de um extrato real completou 200, com `months`/`total_transactions`/`files` corretos na resposta.
- **BUG REAL, já corrigido no Syncron Core (não neste repositório) — `404` (`no_data_yet`) e `410` (`work_session_expired`) estavam sendo tratados como o mesmo erro genérico pelo lado consumidor.** O ANZ Finance já separa essas duas situações deliberadamente desde a Fase 13 (ver CLAUDE.md — sessão nunca recebeu dado vs. sessão recebeu dado mas o workspace TTL em memória expirou), mas o adapter do Core inicialmente relatava as duas como "nunca enviou nada", o que é enganoso justamente no caso mais comum (usuário que já enviou e só esbarrou no TTL). Corrigido no lado do Core para respeitar a distinção que este backend já expõe corretamente — nenhuma mudança necessária aqui.
- **SUSPEITA REAL — `POST /ai/ask` retornou `502 {"detail":{"message":"O provedor de IA não respondeu. Tente novamente em instantes."}}` num teste ao vivo, imediatamente após um upload bem-sucedido no mesmo workspace.** Testado diretamente contra a produção (bypassando o Core) com um script Python que faz upload e pergunta em sequência rápida no mesmo processo — o upload e a criação/leitura do workspace funcionam corretamente (descarta problema de afinidade de processo/múltiplos workers, já que não há nenhuma camada de persistência entre eles — ver guardrail #1 do CLAUDE.md), mas a chamada ao provedor de IA (`provider=groq` por padrão) falhou de forma reproduzível. Mesma classe de sintoma já confirmada como bug real no Docksmith (`GROQ_API_KEY` de produção sem acesso aos modelos anunciados) — forte suspeita de que a `GROQ_API_KEY` configurada no Railway deste serviço tem o mesmo problema (inválida, revogada, ou sem acesso ao modelo default), mas isso **não foi confirmado diretamente aqui** (não tentei nenhum outro provider nem inspecionei a chave configurada — fora do escopo da integração do Core, que corretamente reporta esse 502 como falha, não como sucesso silencioso). Vale checar a `GROQ_API_KEY` configurada no Railway deste serviço, e considerar `OPENAI_API_KEY` como fallback já que o backend suporta os dois providers (ver Fase 5 do CLAUDE.md).
